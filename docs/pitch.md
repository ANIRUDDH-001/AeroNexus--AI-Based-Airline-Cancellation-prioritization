...
| 1–2 | What we built | Overview page, demo day loaded, decision time 05:00 | "A synthetic IndiGo-like day: 259 flights, 40 aircraft, fog at Delhi plus an aircraft technical. If nobody acts: 6 forced cancellations, 700 stranded." |
| 2–4 | The recommendation | Recommendations → *Recommend* | "Under five seconds. Three plans, each simulated through the whole day. Plan 1 swaps one aircraft and cancels two cycles early: 1 forced cancellation instead of 6, 550 fewer stranded. Every bullet is computed, not generated." Open *why over #2* and the confidence readout. |
...
| 7–8 | Proof | Cases & benchmarks page | "Twelve hand-written situations with a known right answer — all pass. And a ladder: naive rule, weighted heuristic, AeroNexus — scored on futures the search never saw. 26 % lower network impact, 83 % win rate, forced cancellations cut from 5.6 to 2.1." |
...
* *Latency at IndiGo scale (2,000 flights)?* — Search cost scales with at-risk flights, not fleet size; candidate cap M and beam K bound it; the surrogate and latency budget degrade gracefully (fewer samples, shallower depth) and the run says so.
...
**Demo safety**
* Warm the Render service and un-pause Supabase the evening before; open `/health`.
