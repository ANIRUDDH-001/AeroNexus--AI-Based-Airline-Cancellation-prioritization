  ...
  return (
    <>
      ...
      <PageTitle
        ...
        subtitle="Each policy's plan is scored on held-out futures: the same simulator, a different random seed and ±5 % block-time noise the search never saw. This guards against fitting the sampled futures, not against errors in the model itself."
        right={
      ...
      <Card className="mt-8">
        <div className="font-medium mb-1">What these results do and do not show</div>
        <ul className="text-[13px] text-ink-2 space-y-1">
          <li>• All days are generated by a seeded synthetic model of an IndiGo-like network. No real schedules, crews, passengers or disruption histories were used.</li>
          <li>• Compensation, hotel and revenue terms use indicative tariffs; DGCA compensation and FDTL tables are placeholders marked VERIFY in the configuration.</li>
          <li>• The evaluation futures come from the same simulator as the search, so the ladder measures search quality under uncertainty, not model accuracy against real operations.</li>
          <li>• Percentages are averages over the scored days; per-day results (below) vary, and the engine is occasionally worse than waiting under very uncertain fog.</li>
          <li>• Latency figures depend on the machine: the hosted engine is slower than a laptop and reduces samples and depth to stay within budget (shown on every run).</li>
        </ul>
      </Card>
    </>
  );
...
function BenchmarkView({ bench }: { bench: Benchmark }) {
  ...
  const dn = bench.summary.do_nothing;
  const vsDoNothing = b2 && dn ? 100 * (1 - b2.nis_mean / Math.max(dn.nis_mean, 1e-9)) : null;
  return (
    <>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <Kpi label="Generated days with at-risk flights" value={bench.scenarios} />
        <Kpi label="Held-out futures per plan" value={`${bench.s_eval} · block noise σ ${bench.block_sigma}`} />
        {vsDoNothing != null && <Kpi label="AeroNexus vs doing nothing" value={`${vsDoNothing.toFixed(1)} % lower NIS`} tone={vsDoNothing > 0 ? "ok" : "bad"} />}
        {b2 && b0 && (
          <Kpi label="vs naive rule B0 · win rate" value={`${b2.improvement_vs_B0_pct.toFixed(1)} % · ${Math.round(b2.win_rate_vs_B0 * 100)} %`} tone={b2.improvement_vs_B0_pct > 0 ? "ok" : "bad"} />
        )}
      </div>
      <p className="text-[12px] text-ink-2 mb-3">
        Read this as: on generated days, scored on futures the search never saw. The naive rule (cancel the at-risk flight with the fewest passengers) is often worse than doing nothing, so the
        comparison against <em>doing nothing</em> is the conservative one.
      </p>
      <p className="text-[12px] text-ink-2 mb-3">
